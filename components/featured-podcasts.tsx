  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[1, 2, 3, 4].map((i) => (
          <Card key={i} className="overflow-hidden animate-pulse dark:bg-slate-900">
            <div className="aspect-square bg-gray-200 dark:bg-slate-800"></div>
            <CardContent className="p-4">
              <div className="h-3 bg-gray-200 dark:bg-slate-800 rounded mb-1"></div>
              <div className="h-4 bg-gray-200 dark:bg-slate-800 rounded mb-1"></div>
              <div className="h-3 bg-gray-200 dark:bg-slate-800 rounded mb-2 w-2/3"></div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {podcasts.map((podcast) => (
        <Link href={`/podcasts/${podcast.id}`} key={podcast.id}>
          <Card className="overflow-hidden transition-all hover:shadow-md dark:bg-slate-900">
            ...
            <CardContent className="p-4">
              <div className="text-xs font-medium text-primary dark:text-cyan-400 mb-1">
                {podcast.genre?.name || podcast.category?.replace('_', ' ') || t('uncategorized')}
              </div>
              <h3 className="font-semibold text-lg mb-1 line-clamp-1 dark:text-white">{podcast.title}</h3>
      ...
      {podcasts.length === 0 && !isLoading && (
        <div className="col-span-4 text-center py-8">
          <p className="text-muted-foreground dark:text-gray-400">{t('noPodcastsAvailable')}</p>
        </div>
      )}
    </div>
  )
